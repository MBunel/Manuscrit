...
La création d'une \emph{zone de localisation probable} à partir d'un
ensemble \emph{d'indices de localisation,} tels que \enquote{Je vois
  un lac et je suis sur un chemin}, nécessite deux étapes, la
\emph{spatialisation} des \emph{indices de localisation} () et la
\emph{fusion} des \emph{zones de localisation compatibles} en
résultant. Comme nous l'indiquions dans le \autoref{chap:02} ces deux
étapes sont à la base de notre méthode.


, il est
possible de les combiner de manière différente.

Une première solution, illustrée par la figure
\ref{fig:comp_approches_lin}, consiste à les \enquote{chainer}, de
manière à ce que le résultat de la \emph{spatialisation} d'un
\emph{indice} réduise la zone de recherche de \emph{l'indice} suivant.
%
Avec cette approche les différentes \emph{zones de localisation
