...
\tdi{Est-ce que ce ne serait pas mieux d'ajouter ici la formalisation
  des indices de localisation (originalement prévue pour le chapitre
  suivant ?}
...
\begin{quote}
  \begin{tikzpicture}[%
    ...
    \node[anchor=south west,text=RdBu-9-1] {$\wedge$}
    child {node[anchor=south west] {\enquote{je suis proche du \emph{Pic
            de Jean Ray}}}}
    child {node[anchor=south west, text=RdBu-9-9] {$\vee$}
      child{node[anchor=south west, text=RdBu-9-1] {$\wedge$}
