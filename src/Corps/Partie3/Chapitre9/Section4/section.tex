...
\begin{figure}
  ...
  \caption{\ac{zlc} spatialisant la zone atteignable en moins de 12h
    de marche \protect\subref{ZLC_1_TempsMarche} et \ac{zlc}
    spatialisant la zone atteignable en plus d'une demi-heure de
    marche \protect\subref{ZLC_2_TempsMarche} }
...
\ac{zir} à l'ensemble des objets de référence. Environ \num{1500}
lignes électriques, provenant du portail \emph{opendata} d'Enedis, ont
donc été rajoutées aux \num{34} instances de lignes électriques
précédemment prises en compte.


La \autoref{fig:ZLC_FilRouge_6_2} représente une actualisation de la
\autoref{fig:ZLC_FilRouge_6} après l'ajout des lignes opérées par
Enedis.
...
La \autoref{fig:zlp_fil_rouge_2} présente une actualisation de la
\ac{zlp}, après modification de l'indice de localisaiton : \enquote{La
  victime est sous une ligne électrique trois brins} (\ref{ind:fr6}).

\begin{figure}
  \centering
  \input{../figures/ZLP_V2_FilRouge/ZLP.tex}
  \caption{\ac{zlp} de l'alerte \emph{fil rouge} après }
