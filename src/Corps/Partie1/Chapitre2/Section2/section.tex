...
Le développement d'une méthode de \emph{spatialisation} des
...
caféiers. C'est le contexte d’utilisation, ou le mot \enquote{café}
est couplé à une \emph{relation de localisation} qui permet de
comprendre qu'il s'agit d'un objet géographique et donc que
...
des \emph{indices de localisation,} de la forme : \enquote{Je suis
  \emph{sur} \textins{un chemin, une route, un sentier,
    \emph{etc.}}}. Il nous est donc impossible d’élaborer notre
méthode de \emph{spatialisation} uniquement à partir de données issues
des alertes passée.
