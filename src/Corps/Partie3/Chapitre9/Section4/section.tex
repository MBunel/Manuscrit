...
nécessite donc deux objets de référence, \texttt{X} et \texttt{Y}, le
premier donnant le point visé et le second le point de départ. Comme
la relation de localisation \onto[orl]{Entre\-X\-Et\-Y} \footnote{Que
  nous avons déjà utilisée pour modéliser le \href{cinquième
    indice}{ind:gv5} de l'alerte \emph{Grand Veymont.}} la \ac{zlc}
correspondant a cette relation de localisation est symétrique,
c'est-à-dire que l'inversion des objets \texttt{X} et \texttt{Y}
n'impacte pas le résultat de la spatialisation.
...
pouvant être un objet de référence potentiel.
