...
logique est nécessairement vraie ou fausse \footnote{La notation
  ...
  à cette formulation.} On pourrait penser que la \emph{logique floue}
est fondamentalement incompatible avec ces deux principes, une même
proposition pouvant être à la fois (partiellement) vraie et fausse
\footnote{C'est par exemple le cas dans l'exemple illustré par le
  ...
  cas de leur négation respective.}.


\multiplefootnoteseparator \footnote{Ce
  principe ne doit pas être confondu avec le principe de
  \emph{non-bivalence} à la base de la \emph{logique floue.} Une
  logique peut être non-bivalente, tout en respectant le principe du
  \emph{tiers-exclu} (\eg la logique ternaire de \bsc{Łukasiewicz}).}
\autocite{Horn2018}. 
