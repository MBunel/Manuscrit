...
tout d'abord il faut identifier une \emph{métrique}
...
éloigné de \emph{l'objet de référence.} Ainsi, une position proche de
\emph{l'objet de référence} doit avoir un \emph{degré d'appartenance}
à la \emph{zone de localisation compatible} correspondant à cette
\emph{relation de localisation atomique} plus élevé qu'une position
qui en est éloigné. Le degré d'appartenance doit donc diminuer lorsque
la distance à \emph{l'objet de référence} (\ie la valeur de la
métrique) augmente.

De plus, la notion de proximité sous-entendue par
cette \emph{relation de localisation atomique} 
