...
l'archimédianté. Un couple d'opérateurs est dit archimédien lorsqu'il
est continu et que, pour toute valeur de degré d’apparence (\(x\))
entre 0 et 1 (exclus), la \emph{t-norme} de \(x\) avec lui-même et
inférieure à \(x\) et la \emph{t-conorme} de \(x\) avec lui-même est
supérieure à \(x\), soit : \(\forall x \in ]0,1[\) : \(⊤_L(x,x) < x\)
et \( ⊥_L(x,x) > x\) \autocite{Bouchon-Meunier1995}.
...
Cette propriété a un impact conséquent sur le résultat des
intersections et des unions. Il est donc nécessaire de les
