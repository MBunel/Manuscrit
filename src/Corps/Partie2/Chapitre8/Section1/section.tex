...
et des unions. Le choix des opérateurs utilisés durant \emph{la phase
  de fusion} ne doit donc pas se fonder uniquement sur la sévérité que
l'on veut donner aux différentes étapes de la \emph{fusion,} mais
également sur les différentes propriétés des opérateurs et leur(s)
impact(s) sur la sémantique de la \emph{fusion.}  Nous allons donc
détailler les différentes propriétés des opérateurs de \emph{fusion}
et étudier leur influence sur la \emph{fusion} des \ac{zlc}, en vue de
choisir le couple d'opérateur le plus adapté à notre méthode.
