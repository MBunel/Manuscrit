...
\(⊥(x,x) > x\) \autocite{Bouchon-Meunier1995}. De plus, si la
\emph{t-norme} et la \emph{t-conorme} sont continues et que, pour tous
les degrés d'appartenance \(x\), \(y\), \(v\) et \(w\) tels que \(x\)
et strictement inférieur à \(v\) et \(y\) et strictement inférieur à
...
couple \(v, w\), \ie que pour tout \(x < v\) et \(y < w\) :
\(⊤(x,y) < ⊤(v,w)\) et \(⊥(x,y) > ⊥(v,w)\), alors les opérateurs sont
dis archimédiens stricts \autocite{Bouchon-Meunier1995}.
