...
ensemblistes à la théorie des sous-ensembles flous. Les opérateurs que
nous avons utilisés jusqu'ici, notamment lors de la définition de la
représentation des \emph{zones de localisation} (\autoref{chap:06}),
étaient les opérateurs dit de \bsc{Zadeh} \footnote{Voir les équations
...
\autocite{Zadeh1965}. Les opérateurs de \bsc{Zadeh} sont parfaitement
adaptés à la réalisation d'intersection et d'unions
inter-sous-ensembles flous et donc à la \emph{phase de fusion,} mais
les opérateurs alternatifs (\ie les opérateurs de \bsc{Łukasiewicz},
probabilistes et drastiques), permettent de modifier le résultat des
étapes de fusion, en proposant des intersections et des unions plus ou
moins sévères (\autoref{chap:03}). L'utilisation d'opérateurs
alternatifs pose cependant des problèmes spécifiques, chacun
d'entre-eux ayant des propriétés spécifiques (différentes des
opérateurs de \bsc{Zadeh}), impactant le résultat des intersections et
des unions. Le choix des opérateurs utilisés durant \emph{la phase de
  fusion} ne doit donc pas se fonder uniquement sur la sévérité des
opérateurs, mais également sur leurs différentes propriétés et leur
impact sur la sémantique des différentes étapes de \emph{fusion.} Nous
allons donc détailler les différentes propriétés des opérateurs de
\emph{fusion} et étudier leur influence sur la \emph{fusion} des
