...
\dict{\onto{ApresJalonSurItineraire}}{La \emph{cible} est située sur
  ...
  \onto{Si\-tue\-Sur\-Iti\-ne\-rai\-re\-Ou\-Re\-seau\-Sup\-port}), et
...
\dict{\onto{ATempsDeMarcheDe}}{La \emph{cible} est à tel temps de
  marche du \emph{site,} en marchant depuis le \emph{site} vers la
  \emph{cible.}}{\enquote{On est à 10 minutes du sommet de la
    Bastille.}}%

\dict{\onto{AuDessusALAplombDe}}{La \emph{cible} est à une altitude
  supérieure à celle du \emph{site} et le \emph{site} est plus ou
  moins situé sur la ligne de plus grande pente qui passe par la
  \emph{cible,} ou inversement. Typiquement, \emph{site} et
...
\dict{\onto{AuDessusDeJalonSurItineraire}}{La \emph{cible} est
  au-dessus du \emph{site} et sur un chemin menant au site
  ($\Rightarrow$
...
\dict{\onto{AuMilieuDe}}{Le \emph{site} a une emprise spatiale
  linéaire ou surfacique. La \emph{cible} est située à l’intérieur du
  \emph{site} ($\Rightarrow$ \onto{DansPlanimetrique}), suffisamment
  loin de ses frontières pour qu’on ne puisse pas considérer qu’elle
  en est au bord.}{}%
...
\dict{\onto{AuxAlentoursDe}}{La \emph{cible} est suffisamment proche
  du \emph{site} pour qu’on puisse considérer que le \emph{site} est
  un point de repère qui a un sens, mais potentiellement suffisamment
  loin pour qu'on ne puisse pas considérer qu'on est nécessairement
  dans le cas \onto{Proximal} ou \onto{PresDe}. La proximité qui
  pourrait s’en déduire est généralement dépendante d’un
  \enquote{rayonnement} qui pourrait êtrre affecté au \emph{site} (lié
  à sa renommée, à sa taille typiquement pour un lieu habité, à sa
  saillance, \emph{etc.}). Il peut y avoir connexion topologique entre
  la \emph{cible} et le \emph{site,} ou non.}{\enquote{Dans le massif
    de la Char\-treu\-se}}%

\dict{\onto{AvantJalonSurItineraire}}{La \emph{cible} est située sur
  un itinéraire ($\Rightarrow$
  \onto{Situe\-Sur\-Itineraire\-Ou\-Re\-seau\-Sup\-port}) et avant le
...
\dict{\onto{AvoirASaGauche}}{La \emph{cible} est munie d’un
  référentiel des directions intrinsèque : elle a une gauche et une
  droite (et un avant, un arrière). Dans ce référentiel des
  directions, et plus précisément sur l’axe gauche-droite de ce
  référentiel, le site est situé à gauche de la cible (donc la
  \emph{cible} a le \emph{site} à sa gauche, au sens commun de
  l’expression).}{}%

\dict{\onto{AvoirDerriereSoit}}{La cible est munie d’un référentiel
  des directions intrinsèque : elle a un avant et un arrière, (et une
  gauche, une droite). Dans ce référentiel des directions, et plus
  précisément sur l’axe avant-arrière de ce référentiel, le site est
  situé derrière la cible (donc la cible a le site derrière elle, au
  sens commun de l’expression).}{}%
...
\dict{\onto{DansLaDirection\-De\-X\-DepuisY}}{Cette relation de
  localisation nécessite deux \emph{sites} : $X$ qui fixe la direction
  et Y qui donne le point d'origine de cette direction. Cette relation
  de localisation indique que la \emph{cible} est située quelque part
  \enquote{sur un cheminement entre $X$ et $Y$}. Cette relation
  diffère de \onto{EntreXetY} pour deux raisons : (1) il y a une
  tolérance au dépassement dans l'axe du segment $[XY]$, à une
  extrémité ou à l'autre, pour tenir compte des éventuels détours
  nécessaires selon le support du cheminement (espace homogène
  \emph{vs} muni d'un réseau, etc.); (2) dans la direction orthogonale
  à l'axe $[XY]$, la \emph{cible} est a priori davantage susceptible
  d'être éloignée du segment à l'aplomb du milieu de celui-ci qu'à ses
  extrémités}{}%
...
\dict{\onto{Dans\-Planimetrique}}{La \emph{cible} est située dans
  l’espace 2,5 D délimité par le \emph{site,} qui est donc considéré
  comme délimitant une région sur la surface terrestre.  Cette
  relation de localisation est mutuellement exclusive avec
  \onto{HorsDePlanimetrique}, mais n'en est pas le complémentaire
  (cf. \onto{A\-La\-Fron\-ti\-e\-re\-De}, qui est compatible avec
  \onto{Hors\-De\-Pla\-ni\-me\-tri\-que}, avec
  \onto{DansPlanimetrique}, ou avec une situation ou ni l'un ni
  l'autre ne s'applique).}{\enquote{Non, on est en forêt.}}%

\dict{\onto{DeAutreCoteDeXParRapportAY}}{La \emph{cible} est de
  l’autre côté du \emph{site} $X$ par rapport au \emph{site}
  $Y$. Autrement dit, la \emph{cible} et le \emph{site} $Y$ sont de
  part et d’autre du \emph{site} $X$. C’est une situation opposée à
  \onto{Du\-Meme\-Cote\-Que\-Par\-Rapport\-A}, où la \emph{cible} et
  le \emph{site} $X$ sont situés du même côté du \emph{site}
  $Y$. Comme pour la relation
  \onto{Du\-Meme\-Cote\-Que\-Par\-Rapport\-A}, cela suppose qu’on peut
  partitionner l’espace en deux zones appelées les deux
  \enquote{côtés} du \emph{site} $X$. Selon la forme et la nature du
  \emph{site} $X$ (et son contexte spatial) :
  %  
  \begin{enumerate*}[label=(\arabic*)]
  \item soit le \emph{site} $X$ a intrinsèquement deux côtés (un col,
    une crête, une rivière, une ville située sur une rivière,
    \emph{etc.}),
  \item soit, pour deux \emph{sites} assimilables à des ponctuels et
    sans contexte spatial particulier, c’est le couple (\emph{site} $X$,
    \emph{site} $Y$) qui permet de définir deux côtés au \emph{site} $X$ :
    un côté qui contient le \emph{site} $X$, et un côté qui ne le
    contient pas.
  \end{enumerate*}
  Typiquement, la limite est alors la perpendiculaire au segment
  joignant le \emph{site} $X$ au \emph{site} $Y$, passant par le
  \emph{site} $X$.}{\enquote{Les rochers de l’hom\-me, vous descendiez
    de l’autre côté plutôt ?}}%

\dict{\onto{ADistanceQuantitativePlanimetrique}}{La \emph{cible} est à
...
\dict{\onto{DuMemeCoteQueXParRapportAY}}{La \emph{cible} située du
  même côté que le \emph{site} $X$, par rapport au \emph{site} $Y$. Cela
  suppose qu’on peut partitionner l’espace en deux zones appelées les
  deux \enquote{côtés} du \emph{site} $Y$. Selon la forme et la nature
  du \emph{site} $Y$ (et son contexte spatial) :
  %
  \begin{enumerate*}[label=(\arabic*)]
  \item soit le \emph{site} $Y$ a intrinsèquement deux côtés (un col,
    une crète, une rivière, une ville située sur une rivière,
    \emph{etc.}),
  \item soit, pour deux \emph{sites} assimilables à des ponctuels et
    sans contexte spatial particulier, c’est le couple (\emph{site} $X$, \emph{site}
    $Y$) qui permet de définir deux côtés au \emph{site} $Y$ : un côté qui
    contient le \emph{site} $X$, et un côté qui ne le contient pas.
  \end{enumerate*}
  Typiquement, la limite est alors la perpendiculaire au segment
  joignant le \emph{site} $X$ au \emph{site} $Y$ et passant par le
  \emph{site} $Y$. Cette relation est l’exact opposé de
...
\dict{\onto{EntreXetY}}{Relation ternaire, nécessitant deux
  ...
  \autocite{Clementini2006}. Mais attention aux cas déictiques comme
  \enquote{je vois $Z$ entre $X$ et $Y$} ou \enquote{nagez entre les
    deux poteaux} de \autocite{Bateman2010}.}{\enquote{Là on est dans
    le secteur entre \textelp{} Cordéac et Pellafol.}}%

\dict{\onto{HorsDePlanimetrique}}{La \emph{cible} est située en dehors
  de l’espace 2,5 D délimité par le \emph{site,} qui est donc
  considéré comme délimitant une région sur la surface
  terrestre. Cette relation de localisation est mutuellement exclusive
  avec \onto{DansPlanimetrique}, mais n'en est pas le
  complémentaire.}{}%

\dict{\onto{PresDe}}{La \emph{cible} est \enquote{proche} du
  \emph{site} (distance qualitative, forcément subjective), mais pas
  dans le \emph{site} (la relation de localisation
  \onto{DansPlanimetrique} n'est pas valide) si ce dernier a une
  emprise non ponctuelle. Par ailleurs, la situation se différencie de
  la situation \onto{Proximal} par la distance qui peut être un peu
  plus grande, et par l'absence de relation fonctionnelle. Enfin,
  \onto{DansPlanimetrique} implique \onto{PresDe} mais l'inverse n'est
  pas vrai.}{\enquote{Il était près du sommet et dans le
    brouillard.}}%

\dict{\onto{Proximal}}{La \emph{cible} est dans le \emph{site} ou à sa
  proximité immédiate, qui peut être une proximité
  fonctionnelle.}{\enquote{Elle est à 50 mètres, je suis sur un
    promontoire car le réseau ne passait pas.}}%
...
\dict{\onto{Si\-tue\-Sur\-Iti\-ne\-rai\-re\-Ou\-Re\-seau\-Sup\-port}}{La
  ...
  randonnée, \emph{etc.} Aucun a priori n’est considéré sur la
...
\dict{\onto{Sous\-A\-L\-A\-plomb\-De}}{La \emph{cible} est à une
  altitude supérieure à celle du \emph{site} et ce dernier est plus ou
  moins situé sur la ligne de plus grande pente qui passe par la
...
\dict{\onto{SousProcheDe}}{La \emph{cible} est proche et a une
  altitude inférieure au \emph{site}.}{\enquote{On est à 100 mètres ou
    150 mètres, ou 200 mètres du col, vraiment juste en dessous.}}%
