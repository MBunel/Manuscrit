...
sous-entend un \emph{recouvrement} (\ie \emph{le pont est entre le
  locuteur et le ciel}), mais non la seconde. Cette différence
s'explique par la nature des \emph{objets de référence.} On peut être
recouvert par \emph{un pont} (ou un arbre, le ciel, \emph{etc.})  mais
pas par une route (ou un bâtiment). Ainsi, il est nécessaire
