...
décomposition a été fixée, il est nécessaire de définir en détail les
\emph{relations de localisation,} les \emph{relations de
  décomposition} et les \emph{relations de localisation atomiques} y
prenant place. Pour ce faire on peut se fonder sur la littérature et
plus particulièrement sur des travaux formalisant des \emph{relations
  de localisation,} \cite{Vandeloise1986,Aurnague1996,Borillo1998}
pour identifier les \emph{relations de localisation} et proposer des
décompositions. Cependant, bien que l'utilisation de travaux
précédents soit essentielle, elle ne nous semble pas être pleinement
adaptée à notre cas d’application. En effet ces travaux traitent des
\emph{relations de localisation} dans leur acceptation générale, alors
que nous travaillons sur un contexte particulier, dont on peut augurer
que la spécificité impacte la sémantique des \emph{relations de
  localisation.}

\subsection{La retranscription des alertes}
...
Les retranscriptions utilisées pour construire \emph{les ontologies
  des relations de localisation} et des \emph{relations de
  localisation atomiques} ont été réalisées à partir d'un ensemble de
52 enregistrements d'appels de demande d'assistance auprès des
\ac{pghm} de Chamonix (5 alertes) et de Grenoble (47 alertes). Ces
...
second en 2019. Ces deux corpus sont relativement similaires, que ce
soit par le nombre d'alertes, leur durée ---~qui varie de quelques
secondes à plusieurs dizaines de minutes
(\autoref{fig:dist_temps_alertes})~--- ou leur
contenu. Malheureusement nous n'avons que peu d'informations sur le
contexte de ces différentes alertes. Si nous savons que ces dernières
ne sont qu'une sélection parmi l'ensemble des enregistrements à la
disposition du \ac{pghm} de Grenoble, nous n'avons pas connaissance de
la manière dont cette sélection a été effectuée, bien que ces alertes
aient été qualifiées de représentatives par le \ac{pghm}.
