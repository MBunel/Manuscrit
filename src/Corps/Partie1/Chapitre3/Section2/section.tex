...
Ces différents couples d'opérateurs ont des caractéristiques
différentes et le choix de l'un d'entre eux doit se faire en fonction
de ces caractéristiques. Comme nous l'indiquions précédemment, les
opérateurs de \textcite{Zadeh1965} constituent le \enquote{choix par
  défaut}, leurs propriétés étant assez proches des opérateurs d'union
et d'intersection utilisés dans la théorie des ensembles. Ces
opérateurs ne permettent cependant pas de satisfaire les lois de
\emph{non-contradiction} et du \emph{tiers exclu,} ce qui peut poser
problème pour certaines applications. À l'inverse, d'autres opérateurs
comme les opérateurs de \bsc{Łukasiewicz} ou probabilistes sont
\emph{archimédiens,} ce qui modifie la sémantique de la
\emph{conjonction} et de la \emph{disjonction} logique, en valorisant
la \emph{conjonction} de deux valeurs de degré d'appartenance
similaire (et pénalisant la \emph{disjonction}). Ainsi le choix d'une
famille d'opérateur doit ce faire en connaissance de cause, en
fonction des caractéristiques que l'on souhaite voir conservées.
