\begin{tikzpicture}
  ...
  % 
  \node[inner sep=0pt, anchor=south west] (image) at (0,0){\includegraphics{./figures/EcartNord_PasVille.png}};
  % 
  \begin{scope}
    ...
    \begin{scope}
      ...
      {
        \draw[fill=RdBu-9-1,draw=none, below] ([xshift=\xshift cm, yshift=-2cm]P1) circle [radius=\rad cm];
      }
      \foreach \x [evaluate=\xshift using 1+\x/10, evaluate=\rad using (\x * .0004) + .01] in {0,...,50}
      {
        \draw[fill=RdBu-9-9,draw=none, below] ([xshift=5+\xshift cm, yshift=-2cm]P1) circle [radius=\rad cm];
      }
      % 
      \path(1,-2.5) --++ (10,0)
    ...
    % Légende détaillée
    \path (P1) -- (P2) node[pos=.5, yshift=-3.5cm] {\tiny Pour la légende détaillée du fond topographique voir \autoref{anx:topo_leg}. Sources: BD TOPO 2018, BD ALTI 2018.}; 
  \end{scope}
\end{tikzpicture}