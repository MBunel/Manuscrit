...
Le travail d'identification de la sémantique des \emph{relations de
  localisation,} présenté précédemment (obj. \ref{subsec:2-1}), se
...
cherchons à sémantiser sont exprimées en langage naturel et donc,
soumises à l'interprétation du locuteur. Par conséquent, la
\emph{limite} entre la zone qui correspond à un \emph{indice de
  localisation} et celle qui n'y correspond pas, n'est pas
nécessairement, voire jamais, une ligne. Les conséquences de ce fait
sont multiples. D'une part, cela complique la formalisation des
...
\emph{classique} \footnote{C'est-à-dire, telle que formulée au début
  du \bsc{xix}\up{ème} siècle, principalement par \bsc{Fredge} dans
  son \emph{Idéographie} (\emph{Begriffsschrift}).}, repose sur un
...
raisonnement, calqué sur le \emph{paradoxe sorite}
...
Comme nous venons de le mentionner, la notion \emph{d'imprécison} est
intimement liée aux travaux sur la définition d'un langage formel.
