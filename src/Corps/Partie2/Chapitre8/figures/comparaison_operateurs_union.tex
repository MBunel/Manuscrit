  ...
  \begin{scope}[xshift=2cm,local bounding box=fuzz2]
    \begin{scope}
      \foreach \x in {0,.25,...,1.75}{
        ...
        {
          % Calcul de l'angle
          \calcangle{\angle}{\pgfpoint{1cm}{1cm}}{\pgfpoint{2cm}{0cm}}{\pgfpoint{\x cm}{\y cm}}
          % Fuzzyfication de l'angle
          \pgfmathsetmacro\fuzzy{%
            ifthenelse(\angle < 90,(90-\angle)/90,
            ifthenelse(\angle > 270,(\angle-270)/90,0)
  ...
  \begin{scope}[xshift=-6cm, yshift=-4cm,local bounding box=fus1]
    \begin{scope}
      \foreach \x in {0,.25,...,1.75}{
        ...
        {
          % Calcul des angles
          \calcangle{\angle1}{\pgfpoint{1cm}{1cm}}{\pgfpoint{1cm}{0cm}}{\pgfpoint{\x
              cm}{\y cm}}
          \calcangle{\angle2}{\pgfpoint{1cm}{1cm}}{\pgfpoint{2cm}{0cm}}{\pgfpoint{\x cm}{\y cm}}
          % Fuzzyfication des angles
          \pgfmathsetmacro\fuzzy1{%
            ifthenelse(\angle < 90,(90-\angle)/90,
            ifthenelse(\angle > 270,(\angle-270)/90,0)
            )
          }

          \pgfmathsetmacro\fuzzy2{%
            ifthenelse(\angle2 < 90,(90-\angle2)/90,
            ifthenelse(\angle2 > 270,(\angle2-270)/90,0)
            )
          }

          % fusion (zadeh)
          \pgfmathsetmacro\fus{max(\fuzzy1, \fuzzy2)}
