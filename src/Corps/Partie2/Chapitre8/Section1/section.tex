...
(\autoref{fig:methodo_1}). Ainsi, de la même manière que les
\emph{indices de localisation} sont décomposables à l'aide de
disjonctions et de conjonctions (\autoref{chap:04}), il est possible
de construire la \emph{zone de localisaiton probable,} spatialisant
l'ensemble des indices de localisation initial en appliquant les
mêmes combinaisons logiques entre les \ac{zlc} spatialisant les
...
d'un même type \emph{d'objet de référence.} Cette ce distingue de la
précédente par l'utilisation d'un opérateur conjonctif entre les
différentes \ac{zlc}. En effet, si lors de la fusion des relations de
localisation atomiques on cherche à construire les \ac{zlc} comprenant
toutes les positions correspondant à toutes les \emph{relations de
  localisation atomiques,} ici on cherche à identifier toutes les
positions validant la relation de localisation spatialisée pour au
moins un \emph{objet de référence.}
%
...
initial, c'est-à-dire d'autant de \emph{zones} que l'on a d'indices à
la fin de la première étape de décomposition
(\autoref{fig:methodo_1}).
...
la \ac{zlp}. Contrairement aux précédentes étapes, cette étape crée
toujours une \emph{zone de localisation,} que l'on spatialise un seul
\emph{indice de localisation} utilisant une \emph{relation de
  localisation atomique} et se référant à un \emph{objet de référence}
nommé, ou que l'on spatialise une dizaine d'indices de localisation
complexes. Cette étape utilise, comme la première, un opérateur
conjonctif, permettant d'identifier les pixels appartenant à
l'ensemble des \ac{zlc} combinées. Toutefois il présente une
différence majeure avec la première étape de la phase de
\emph{fusion,} puisqu'elle nécessite de prendre en compte l'évaluation
de la qualité de l'indice de localisation par le secouriste, ce que
détaillerons dans la seconde partie de ce chapitre.
...
particularité d'être discontinu.

Le comportement des opérateurs drastiques est assez particulier et il
ne nous semble pas pertinent de les conserver.
...
\emph{non-contradiction,} fondamentaux en logique classique et
traditionnelle \footnote{Ou aristotélicienne.}  \autocite{Horn2018}. Le
...
\footnote{La notation formelle de ce principe est exprimée par la
  tautologie \(\neg(p \wedge \neg p) \).}. Il est complété par le
...
logique est nécessairement vraie ou fausse \footnote{La notation
  formelle de ce principe est exprimée par la tautologie
  \(p \vee \neg p\).} On pourrait penser que la \emph{logique floue}
...
logique ternaire de \bsc{Łukasiewicz} valide ces deux principes.

Pour qu'un couple d'opérateurs soit conforme au principe de
...
On peut vérifier le comportement des opérateurs en représentant le
...
\onto[orla]{Près\-De}, avec son complémentaire \footnote{Soit la
  \ac{zlc} correspondant à la spatilisation de la \ac{zlc}
  \onto[orla]{Près\-De} avec le \emph{modifieur} \onto[orla]{Not}
  (\autoref{chap:07}).}
(\textcolor{RdBu-9-9}{\textsf{A\up{C}}}). Quelle que soit la figure
...
qu'il soit égal à 1 \footnote{Ce qui correspond à \emph{l'ensemble de
  ...
  qui correspond à l'enseble vide.}
(\autoref{fig:non-contradiction}). À l'inverse, les opérateurs de
\bsc{Zadeh} ou probabilistes aboutissent à une zone non uniforme,
...
de \bsc{Łukasiewicz} et les opérateurs drastiques, les opérateurs de
\bsc{Zadeh} et les opérateurs probabilistes ne valident donc pas les
...
La non-validation de ces deux principes n'est cependant pas une raison
...
\begin{displayquote}
  \textins{Sans le principe de la non-contradiction} Une personne
...
notre cas d’application. En effet notre méthode ne se base pas sur des
raisonnements logiques qui seraient impactés par la non validation des
principes du tiers-exclus et de la non-contradiction. Il n'est donc
pas nécessaire que les opérateurs employés lors de la fusion valident
ces principes, nous ne pouvons donc en disqualifier aucun.

% Ces considérations étant
% assez théoriques et les opérations permettant de les mettre
% directement en évidence (équations \ref{eq:ensemble_LNC} et
% \ref{eq:ensemble_LEM}) n'entrant pas dans la \emph{phase de fusion.}
% On peut toutefois augurer que le non-respect de ces principes puisse
% avoir des effets indirects sur les résultats des différentes
% opérations inter-zones réalisées durant la \emph{fusion,} qu'ils
% conduisent à une amélioration de la modélisation ou, au contraire,
% qu'ils la dégradent. L'importance de la conservation de ces deux
% principes doit donc être jugée à l'aune de leur impact sur la
% modélisation et plus gnéralement sur la sémantique des opérations
% inter-zones.
...
On pourrait cependant penser que cette sensibilité à la taille de
...
\bsc{Łukasiewicz} une intersection de 10 fois la valeur 0,9 donnera un
...
\bsc{Łukasiewicz}, un pixel ayant un degré d'appartenance élevé (0,9)
