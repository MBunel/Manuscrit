...
d'utilisation \autocite[16]{Borillo1998}, comme l'illustre la relation
de localisation \enquote{\emph{sous}}. Comme nous l'expliquions plus
haut (cf. \ref{subsec:2-1-1}), on peut être \enquote{sous \emph{un
    pont}} ou \enquote{sous \emph{une route}}, si la sémantique de ces
deux exemples diffère c'est que le premier cas sous-entend un
\emph{recouvrement} (\ie \emph{le pont est entre le locuteur et le
  ciel}) alors que ce n'est pas le cas pour le second. Cette
différence s'explique par la nature des objets de référence. On peut
être recouvert par \emph{un pont} (ou un arbre, le ciel, \emph{etc.})
mais pas par une route (ou un bâtiment). Ainsi il est nécessaire
d'identifier ces différences sémantiques pour pouvoir
...
construire deux nouveaux \emph{indices de localisation,} mais qui ne
diffèrent que par leur \emph{relation de localisation.}
