Les \emph{zones de localisation} sont des objets qui sont construits
...
\emph{zones de localisation.} Si nous avons détaillé cette notion et
...
\footnote{Formalisée dans l'ontologie des r\emph{elations de
    localisation} par le concept \onto[orl]{Prés\-De}.}, qui décrit
...
\footnote{Comme nous l'avons illustré avec l'exemple du \emph{paradoxe
    sorite} dans le second chapitre (\ref{subsec:2-1-2}).}. Ainsi, les
limites de la \emph{zone de localisation} lui correspondant sont
...
\footnote{Formalisée avec le concept
  \onto[orl]{Sous\-Al\-ti\-tu\-de}.} a des limites plus
...
à 5 qu'à 50 mètres. Cette observation est également vraie pour la
...
dépend des variations du relief qui sont tout sauf linéaires. On
...
proche, voire légèrement supérieure, à celle de \emph{l'objet de
...
correspondant au premier ($\text{\textsf{ZLC}}_{1}$ : \enquote{Je suis
  à côté d'une maison.}) et au second ($\text{\textsf{ZLC}}_{2}$ :
...
l'intersection de $\text{\textsf{ZLC}}_{1}$ avec
$\text{\textsf{ZLC}}_{2}$ pourra donner un résultat différent de
l'intersection de $\text{\textsf{ZLC}}_{2}$ avec
$\text{\textsf{ZLC}}_{1}$. Un tel comportement est incompatible avec
