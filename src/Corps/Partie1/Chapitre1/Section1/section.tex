...
nouveau divertissement \enquote{\textins{d}es grands bourgeois
...
perdent et se retrouvent coincés sur une vire. Exténués et
...
Les victimes ayant été localisées, une opération de secours héliportée
...
notamment grâce à l'introduction de l'\emph{Alouette \bsc{iii}} en
...
secours, ce qui a été infirmé par Michèle \bsc{Alliot-Marie} en 2007,
...
\subsubsection{L'identification assistée de la position des victimes}
...
l'application de géolocalisation \emph{GendLoc.}
...
l’application \emph{GendLoc} n'est pas utilisable.

\paragraph{La solution \emph{GendLoc}}
...
l’application \emph{GendLoc} \footnote{\emph{GendLoc} a
  principalement été développé par M. Olivier
  \bsc{Favre} \autocite{Muscat2015}.} . Contrairement à une application
...
(REGA)~--- \emph{GendLoc} ne demande pas un téléchargement et une
installation préalable. Le principe de fonctionnement de
\emph{GendLoc} consiste à envoyer, par SMS, une URL au
...
d'effectuer une intervention. En effet, \emph{GendLoc} permet de
...
impossible à l'aide de \emph{GendLoc} \footnote{À cet égard, la
  solution proposée par \emph{GendLoc} est moins flexible que
...
\emph{GendLoc.}  Enfin, il est nécessaire que la victime puisse
réaliser l'ensemble des manipulations nécessaires \footnote{%
  ...
  \item valider le partage de la position sur la page web \emph{GendLoc.} 
  \end{enumerate*}}.
%
...
choisissaient fréquemment de ne pas utiliser \emph{GendLoc} dans ces
conditions, préférant rester en contact avec la victime, quitte à
identifier sa position manuellement.

Malgré ces défauts, la solution \emph{GendLoc} est massivement
...
\subsubsection{Les limites des méthodes de localisation des victimes}
...
\emph{GendLoc.} Les secouristes décident donc d'identifier sa
