...
faute d'informations sur l'itinéraire suivit par le requérant.

L'extrait \ref{gv:10} peut, quant à lui, être modélisé avec la
relation de localisation \onto[orl]{Entre\-X\-Et\-Y}, comme nous
l'avons proposé pour l'extrait \ref{gv:4}. On peut cependant constater
que ces deux extraits (\ref{gv:4} et \ref{gv:10}) semblent se
contredire. En effet, si l'on observe la zone de l'alerte
(\autoref{fig:zir_grand_veyont}), on peut remarquer qu'il difficile
identifier des positions à la fois située \enquote{entre} le
\emph{Grand Veymont} et le \emph{Pas de la Ville} et \enquote{entre}
le \emph{Pas de la Ville} et \emph{Pierre Blanche,} ---~les trois
objets géographiques étant quasiment alignés~--- où alors au prix
d'une certaine approximation. L'extrait \ref{gv:11}, bien que non
spatialisable, tend à confirmer la contradiction des extrait
\ref{ind:4} et \ref{ind:10}. En confirmant qu'il est \enquote{au-delà}
du \emph{Pas de la Ville} (\ref{gv:11}) le requérant indique qu'il a
dépassé ce point, alors qu'il arrivait du sommet du \emph{Grand
  Veymont.}

contredit sa
précédente affirmation (\ref{gv:4}), comme quoi il serait situé entre
le \emph{Pas de la Ville} et le \emph{Grand Veymont.}
