...
initialement été réalisée par Ana-Maria \bsc{Olteanu-Raimond} et revue
par Cécile \bsc{Duchêne} et moi-même. Au court de ce processus, 12
...
relation de localisation \onto[orl]{Au\-Nord\-De}, ou une de ses
relations filles, \onto[orl]{Dans\-La\-Partie\-Nord\-De} et
...
\onto[orl]{Au\-Nord\-De}, il est tout à fait possible d'utiliser cette
...
L'information donnée par les extraits \ref{gv:9} et \ref{gv:11} peut
être modélisée à l'aide de la relation de localisation
\onto[orl]{Apres\-Jalon\-Sur\-Itineraire}, permettant de modéliser une
...
\onto[orl]{Sous\-Jalon\-Sur\-Itineraire}, nous ne pouvons pas utiliser
la relation de localisation \onto[orl]{Apres\-Jalon\-Sur\-Itineraire},
faute d'informations sur l'itinéraire suivi par le
requérant. L'extrait \ref{gv:10} peut, quant à lui, être modélisé avec
la relation de localisation \onto[orl]{Entre\-X\-Et\-Y}, comme nous
...
\item \label{ind:gv3} Le requérant est
  ...
  % 
  % \item \label{ind:gv4} Le requérant est \onto[orl]{XX} du \emph{Pas
  %   de la Ville} (\texttt{X}), par rapport au sommet du \emph{Grand
...
\item \label{ind:gv5} Le requérant est \onto[orl]{EntreXetY} le
  ...
  % 
\item \label{ind:gv6} Le requérant est \onto[orl]{Dans\-Planimetrique}
  une zone à peu près plate, avec une confiance normale
  % 
\item \label{ind:gv7} Le requérant est \onto[orl]{Dans\-Planimetrique}
  une zone caillouteuse, avec une confiance normale
  % 
\item \label{ind:gv8} Le requérant est \onto[orl]{Dans\-Planimetrique}
...
% Le requérant est orl#HorsDePlanimetrique du sommet du Grand Veymont
...
localisation \onto[orl]{Hors\-De\-Planimetrique} est, comme la
relation \onto[orl]{Dans\-Planimetrique} à laquelle elle s'oppose
...
\onto[orla]{Sup\-Val\-0} permet quant à elle d'attribuer un degré
...
\begin{figure}
  ...
  \caption{\emph{Rasteriser,} \emph{métrique} et \emph{fuzzyficateur}
    ...
    \protect\onto[orla]{Hors\-De\-Pla\-ni\-me\-tri\-que}.}
...
ponctuel. La métrique \onto[orla]{Distance} calcule donc, pour chaque
...
résultat du calcul de la métrique \onto[orla]{Distance} pour toutes
...
\begin{figure}
  ...
  \caption{Fonction d'appartenance pour le fuzzyficateur
    \protect\onto[orla]{Sup\-Val\-0}, paramétrée.}
  \label{fig:fuzzy_veyont_distanceSommet}
\end{figure}

\begin{figure}
  \centering
  \input{./figures/ZLC_HorsDe_GrandVeymont.tex}
...
\begin{figure}
  \centering
  \input{./figures/Metrique_sous_GrandVeymont.tex}
...
dessous. Ce paramétrage du fuzzyficateur \onto[orla]{Inf\-Val\-0} est
...
\begin{figure}
  \centering
  \input{./figures/fuzzy_veymont_sousSommet.tex}
  \caption{Fonction d'appartenance pour le fuzzyficateur
    \protect\onto[orla]{Sup\-Val\-0}, paramétrée.}
...
\begin{figure}
  \centering
  \input{./figures/ZLC_Sous_GrandVeymont.tex}
...
deux relations de localisation atomiques, \onto[orla]{Au\-Nord\-De} et
\onto[orla]{Hors\-De\-Planimetrique}. La relation de localisation
...
\onto[orla]{Hors\-De\-Planimetrique} permet de modifier ce
...
La première des relations de localisations issues de la décomposition,
\onto[orla]{Au\-Nord\-De} est une relation de localisation cardinale
ayant \onto[orla]{Centroide} comme rasteriser,
\onto[orla]{Ecart\-Angulaire} comme métrique et \onto[orla]{Eq\-Angle}
comme fuzzyficateur
...
La seconde \emph{relation de localisation atomique} est
\onto[orla]{Hors\-De\-Pla\-ni\-mé\-tri\-que}, qui a déjà été employée
...
\onto[orla]{Distance}, qui est ici calculée à partir du ponctuel
...
Pour transformer les métriques \onto[orla]{Ecart\-Angulaire} et
\onto[orla]{Distance} en les \ac{zlc} spatialisant les relations de
localisation atomiques \onto[orla]{Au\-Nord\-De} et
\onto[orla]{Hors\-De\-Pla\-ni\-me\-tri\-que}, il est nécessaire
d'employer respectivement le fuzzyficateur \onto[orla]{Eq\-Angle} et
le fuzzyficateur \onto[orla]{Sup\-Val\-0}. Nous avons fixé le
paramètre \(v\) du premier fuzzyficateur à 0, la métrique
\onto[orla]{Ecart\-Angulaire} calculant un écart à une direction de
...
l'écart est supérieur de \SI{90}{\degree} d'un côté comme de l'autre
(Figure \ref{fig:fuzzy_veyont_angleNord_1}). Pour la fuzzyfication de
la métrique \onto[orla]{Distance}, nous avons employé le même
...
\input{./figures/fuzzy_veymont_angleNord.tex}}\hspace{1cm}
  %
  \subfloat[\label{fig:fuzzy_veyont_angleNord_2}]{%
\input{./figures/fuzzy_veymont_distanceSommet.tex}}
\caption{Fonction d'appartenance pour l'indice de localisation :
  \enquote{Le requérant est \protect\onto[orl]{Au\-Nord\-De\-Externe}
...
\onto[orl]{Au\-Nord\-De} (\autoref{fig:ZLC_Nord_Pas_Ville}) et
\onto[orl]{Hors\-De\-Pla\-ni\-me\-trique}
...
positions \onto[orl]{Au\-Nord\-De\-Externe} du \emph{Pas de la Ville}
...
manières. Tout d'abord, le mode de représentation que nous avons
choisi pour représenter les \ac{zlc} tend à \enquote{gommer} les
variations locales de degré d'appartenance, effet renforcé par
l'échelle de la représentation. Mais cette faible différence met
surtout en évidence le fait que la relation de localisation
\onto[orl]{Hors\-De\-Pla\-ni\-me\-trique} est moins contraignante que
la relation de localisation \onto[orl]{Au\-Nord\-De}.
...
\onto[orl]{Entre\-X\-Et\-Y} le \emph{Pas de la Ville} et \emph{Pierre
...
nommés. La relation de localisation \onto[orl]{Entre\-X\-et\-Y} est,
quant à elle, une relation de localisation décomposable. Elle a la
particularité de décomposer en deux variantes de la relations de
localisation atomique
\onto[orla]{DansLaDirectionA\-Vol\-D\-oiseau\-De\-X\-Depuis\-Y}, la
seconde inversant les objets de référence \texttt{X} et \texttt{Y}. La
relation de localisation
\onto[orla]{DansLaDirectionA\-Vol\-D\-oiseau\-De\-X\-Depuis\-Y} est
une relation de localisation qui permet de généraliser les relations
cardinales (\eg \onto[orl]{Au\-Nord\-De}), pour une direction
donnée. Ainsi définie la relation de localisation
\onto[orl]{Entre\-X\-et\-Y} revient a considérer qu'être entre deux
objets est équivalent à être dans la direction de X depuis Y et
inversement.

Comme la relation de localisation
\onto[orl]{DansLaDirectionDeXDepuisY} est une version générique des
relations de cardinalité, elle partage sa métrique, son fuzzyficateur
et son rasteriser avec ces relations. La
\autoref{fig:Ente_metriques_GrandVeymont} représente le valeur de la
métrique \onto[orla]{Ecart\-Angulaire}, calculée pour ces deux
...
métrique \onto[orla]{Ecart\-Angulaire}, calculée à partir du \emph{Pas
  de la Ville} et par rapport à la demi-droite partant de ce point et
passant par le somme de \emph{Pierre Blanche} et la figure
\ref{fig:Ente_metriques_GrandVeymont_b} représente la valeur de la
métrique \onto[orla]{Ecart\-Angulaire} calculée à partir du sommet de
\emph{Pierre Blanche,} dans la direction du \emph{Pas de la Ville.}

\begin{figure}
  \centering
  \subfloat[\label{fig:Ente_metriques_GrandVeymont_a}]{\includegraphics{../figures/Entre_metrique1_GrandVeymont.png}}\hspace{1cm}
  \subfloat[\label{fig:Ente_metriques_GrandVeymont_b}]{\includegraphics{../figures/Entre_metrique2_GrandVeymont.png}}
  \caption{Métriques calculées pour l'indice de localisation :
    \enquote{La victime est entre le \emph{Pas de la Ville} et
      \emph{Pierre Blanche}}.}
...
\begin{figure}
  ...
  \subfloat[]{\includegraphics{../figures/Entre_1_GrandVeymont.png}}\hspace{1cm}
    ...
    \caption{Zones de localisation compatibles intermédiaires pour
      l'indice de localisation : \enquote{La victime est entre le
...
\onto[orl]{Entre\-X\-Et\-Y} (\autoref{fig:ZLC_GrandVeymont_5}) aboutit
à la construction d'une forme singulière. Les positions situées à
...
\onto[orla]{Eq\-Angle} (ici une valeur de \SI{180}{\degree} a été
utilisée). Plus cette valeur est faible, moins la \ac{zlc}
spatialisant la relation de localisation \onto[orl]{Entre\-X\-Et\-Y}
...
\begin{figure}
  ...
  \caption{Zone de localisation compatible pour le cinquième indice de
    localisation de l'alerte \emph{Grand Veymont :} \enquote{Le
...
localisation \onto[orl]{Dans\-Planimetrique}, mais dans
l'identification des objets de référence. En effet, les
\enquote{zone\textins{s} à peu près plate\textins{s}}, les
\enquote{zone\textins{s} caillouteuses} et les
\enquote{petite\textins{s} prairie\textins{s}} ne sont pas
représentées dans la BD TOPO, ces indices de localisation ne sont donc
pas directement spatialisables. Il est ainsi nécessaire de construire
d'une manière ou d'une autre ces objets de référence. Une première
solution serait d’enrichir les données dont nous disposons avec de
nouvelles sources, comme les données altimétriques (pour identifier
les \enquote{zone\textins{s} à peu près plate\textins{s}}) ou
d'occupation du sol des bases \emph{Corine Land Cover} ou \emph{OCS
  Grande Échelle.} Cependant, la nomenclature proposée par ces bases
de données (et l'échelle dans le cas de la base \emph{Corine Land
  Cover}) n'est pas assez fine pour permettre de distinguer des zones
...
opté pour une autre approche, consistant à exploiter au mieux les
...
contredire. Comme la BD TOPO définit la couverture forestière et que
le requérant indique implicitement qu'il \emph{n'est pas} situé dans
une forêt, nous proposons de transformer ces indices de localisation
en : \enquote{Le requérant n'est pas dans une forêt}. Ce nouvel indice
de localisation est certes moins discriminant que la combinaison des
...
La création de ce nouvel indice de localisation ne modifie pas la
relation de localisation utilisée pour le spatialiser, il s'agit
toujours de la relation \onto[orl]{Dans\-Planimetrique}, déjà utilisée
pour la spatialisation du premier indice de localisation
(\ref{ind:gv1}). Par ailleurs, comme pour cette première utilisation,
il est nécessaire d'utiliser le modifieur \onto[orla]{Not}, qui permet
...
fuzzyficateur est \onto[orla]{Inf\-Val\-0}
...
de la métrique \onto[orla]{Distance} pour deux instances de forêts. La
métrique est ensuite fuzzyfiée, pour chaque objet de référence, à
l'aide du fuzzyficateur \onto[orla]{Inf\-Val\-0} et de son modifieur
\onto[orl]{Not}. Le paramètre \(\delta\) a été fixé 10 mètres, ce qui
implique qu'une position située à plus de 10 mètres de la limite d'une
forêt sera considérée comme en dehors de la forêt.
...
\begin{figure}
  \centering
  \subfloat[\label{fig:metrique_dist_foret_1}]{\includegraphics{./figures/DistanceForetX_GrandVeymont.png}}\hspace{1cm}
  \subfloat[\label{fig:metrique_dist_foret_2}]{\includegraphics{./figures/DistanceForet2_GrandVeymont.png}}
  \caption{Exemples de métriques pour la spatialisation de l'indice de
    localisation \enquote{La victime n'est pas dans une forêt.}}
...
(\ref{ind:gv9}). Une nouvelle fois, nous sommes en présence d'un
indice de localisation qu'il n'est pas nécessaire de décomposer, le
\emph{Pas de la Ville} étant un objet de référence nommé et la
relation de localisation
...
atomique \onto[orla]{Hors\-De\-Planimetrique}, déjà utilisée pour
...
\begin{figure}
  \centering \input{./figures/fuzzy_veymont_distance.tex}
  \caption{Fuzzyficateur \protect\onto[orla]{Eq\-Val} paramétré pour
    le huitième indice de localisation de l'alerte \emph{Grand Veymont
      :} \enquote{Le requérant est
      \protect\onto[orl]{Distance\-Quantitative} de \SI{800}{\meter}
...
\begin{table}
  ...
  \begin{tabular}{clcc}
    ...
    \midrule
    \ref{ind:gv1}&\onto[orla]{Sup\-Val\-0}&\emph{0}&\SI{500}{\meter} \\
    \ref{ind:gv2}&\onto[orla]{Inf\-Val\-0}&\emph{0}&\SI{5}{\meter}\\
    \addlinespace
    \multirow{2}{*}{\ref{ind:gv3}}&\onto[orla]{Sup\-Val\-0}&\emph{0}&\SI{500}{\meter}\\
                   &\onto[orla]{Eq\-Angle}&\SI{0}{\degree}&\SI{180}{\degree}\\

    \addlinespace
    \multirow{2}{*}{\ref{ind:gv5}}&\onto[orla]{Eq\-Angle}&\SI{27}{\degree}&\SI{180}{\degree}\\
                   &\onto[orla]{Eq\-Angle}&\SI{207}{\degree}&\SI{180}{\degree}\\
    \addlinespace
    \ref{ind:gv6}, \ref{ind:gv7}, \ref{ind:gv8}&\onto[orla]{Sup\-Val\-0}&\emph{0}&\SI{10}{\meter}\\
    \ref{ind:gv9}&\onto[orla]{Eq\-Val}&\SI{800}{\meter}&\SI{250}{\meter}\\
    \bottomrule
  \end{tabular}
...
Une fois que les six \ac{zlc} identifiées ont été construites (Figures
...
pouvons construire la \ac{zlp} de l'alerte, correspondant à
...
\begin{figure}
  \centering
  \input{./figures/ZLP_GrandVeymont.tex}
...
à la définition des relations de localisation qui lui auraient semblé
pertinentes. Ainsi, le résultat que nous proposons est sûrement assez
...
la valeur des seuils. En effet, nous n'avons pas réussi à proposer une
démarche permettant de définir les seuils des fuzzyficateurs utilisés
pour la modélisation des \emph{relations de localisation.} Si certains
d'entre-eux sont assez évidents (\eg le fait que la valeur de
...
à ne pas proposer des seuils trop sévères, comptant sur la
...
localisation \enquote{À 800 mètres du \emph{Pas de la Ville}} a été
spatialisé en calculant la métrique \onto[orla]{Distance} à partir du
...
de calculer la \emph{métrique} à partir d'un seul point est une
...
\emph{Pas de la Ville.} S'il peut être satisfaisant de représenter un
...
requérant est une distance quantitative. Exlle est donc fortement
