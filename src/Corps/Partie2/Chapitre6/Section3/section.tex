...
exemple, les \emph{métriques} à utiliser sont assez instinctives.
%
...
la distance euclidienne entre le \emph{sujet} et \emph{l'objet de
  référence} (\autoref{fig:ISO_DIST_HT}).

\begin{carte}
  \centering
  \input{../figures/Raster_ISO_DIST_HT.tex}
  \caption{Distance à la ligne électrique trois brins.}
...
l'altitude de \emph{l'objet de référence} et l'altitude de chaque
position (\autoref{fig:ISO_DALT_HT}). Comme pour la précédente
métrique, se pose la question du point de \emph{l'objet de référence à
  utiliser} pour calculer la métrique.
