...
cependant défini plusieurs relations de localisation dérivées de cette
dernière, comme \onto[orl]{Sous\-Proche\-De}, permettant une
spatialisation plus fine. Cependant, aucune de ces relations ne nous
semble adaptée à la situation. La relation de localisation
\onto[orl]{Sous\-Proche\-De}, contraint l'éloignement au site. Or,
rien dans les extraits de cette alerte ne permet d'affirmer que le
requérant est proche du site, au contraire, des extraits comme le
\ref{gv:1} ou le \ref{gv:5} laissent supposer que le requérant s'est
déjà considérablement éloigné du sommet du \emph{Grand Veymont.} De la
même manière, la relation de localisation
...
la même manière, on ne peut être entre le sommet du \emph{Grand
  Veymont} et le \emph{Pas de la Ville} et en même temps, au-delà du
\emph{Pas de la Ville} en étant parti du sommet du \emph{Grand
  Veymont.} Ces différents extraits se contredisent.

Si l'on considère que les extraits \ref{gv:9} à \ref{gv:11} sont plus
dignes de confiance que l'extrait \ref{gv:4}, ce qui nous semble être
le cas, compte-tenu du fait que le secouriste s'en soit assuré, alors
on doit rejeter l'indice de localisation défini à partir de l'extrait
\ref{gv:4}, ce qui invalide notre précédente interprétation des
extraits \ref{gv:6} et \ref{gv:8}. Les extraits \ref{gv:9} à
...
relation de localisation de localisation \onto[orl]{Au\-Nord\-De}. On
peut cependant raffiner la modélisation en utilisant les relations
filles de \onto[orl]{Au\-Nord\-De},
\onto[orl]{Dans\-La\-Partie\-Nord\-De} et
...
(extraits \ref{gv:9} et \ref{gv:11}) indiquent que le requérant est
au-delà du \emph{Pas de la Ville,} ce qui implique qu'il ait dépassé
cet objet de référence, ce qui permet d'utiliser la relation de
localisation \onto[orl]{Au\-Nord\-De\-Externe}.
