...
employant les relations de localisation \onto[orl]{Aux\-Alentours\-De}
...
\onto{A\-Portee\-De\-Son\-De} pour retranscrire cette indication. Une
...
\onto[orla]{Geometrie}, la métrique \onto[orla]{Temps\-Marche} et la
fuzzyficateur \onto[orla]{Eq\-Val} prenant comme paramètre la durée de
déplacement
...
\begin{figure}[htb]
  ...
  \caption{Métrique \protect\onto[orla]{Temps\-Marche} calculée à
...
la journée. La valeur \(v\) \footnote{Pour rappel, cette valeur
  défini, pour le fuzzyficateur \onto[orla]{Inf\-Val}, dans la borne
...
localisation \onto[orla]{Proximal}. La métrique utilisée par cette
seconde relation est \onto[orla]{Distance\-Euclidienne}. La figure
...
avons laissé le paramètre \(v\) a sa valeur précédente de
...
représenta la \ac{zlc} résultante de cette opération pour un chemin
donné.
...
métrique \onto[orla]{Distance\-Temps}. Cependant lors du calcul de la
...
Une autre critique que l'on pourrait faire à la modélisation proposée
...
sélectionnés. Cette décision nous à conduit à ne pas traiter tous les
...
mais également car, dans le cas présent, les position qui auraient été
prises en compte par les objets de référence retirés, se voient
attribuer un degré d'appartenance nul par d'autres indices. Ainsi, la
\ac{zlp} n'est pas modifiée par de retrait de ces \emph{objets de
  référence.}
...
état actuel, cette modélisation n'arrivait pas à identifier la
