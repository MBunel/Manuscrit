...
\onto{Eq\-Val} est donc définie de la manière suivante :
...
\onto{Eq\-Val} une variante de ce \emph{fuzzyficateur,} dont la
...
manière que les autres \textrm{fuzzyfieurs,} \onto{Inf\-Val} dispose
d'une variante, \onto{Inf\-Val\-0} dont la \emph{valeur de référence}
...
permet cependant pas de traiter toutes les situations rencontrées dans
notre corpus d'alertes. En effet, il n'est pas vraiment possible de
formaliser des \emph{indices de localisation} ayant une tournure
...
est, par exemple, pas prévu dans l'ontologie \ac{orl} de définir une
