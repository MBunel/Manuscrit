...
possible : \(∀ A,B ∈ P(X), Π(A ∩ B) ≤ \min(Π(A), Π(B))\). Par
conséquent, deux événements peuvent avoir chacun une possibilité non
nulle, mais leur occurrence simultanée peut être nulle
(\( Π(A ∪ B) = 0\) n'implique pas que \(Π(A)\) et \(Π(B)\) soient
égaux à 0) \autocite{Bouchon-Meunier2007}. De plus, la \emph{mesure de
  possibilité} est \emph{monotone} relativement à l'inclusion des
parties de \(X)\, c'est-à-dire que si un événement \(A\) est inclus
dans un événement \(B\), alors la possibilité de \(B\) est supérieure
à celle de \(A\) : \(A \subseteq B, Π(A) ≤ Π(B)\). Enfin, tous les
événements de \(X\) ou leur complémentaires sont \enquote{tout à fait
  possibles} : \(∀ A ∈ P(X), \max(Π(A), Π(A^C)) = 1\), par conséquent,
la somme des possibilité des ces deux événements peut dépasser 1 :
\(Π(A) + Π(A^C) ≥ 1\).
