...
l'interface développée au sein du lot 4
(cf. \ref{subsec:1-2-3-3}). Par conséquent nous pouvons développer une
...
modélisée comme une relation de \emph{proximité} où une \emph{distance
  métrique quantitative.} De manière analogue il revient au secouriste
d'analyser l'objet de référence. Dans notre exemple, \emph{l'objet de
  référence} (\enquote{une maison}) désigne un type d'objet (\ie un
\emph{géotype}) auquel plusieurs objets (\ie les \emph{géons}) peuvent
...
supplémentaire tout objet géographique correspondant à la section peut
être considéré comme un objet de référence valide. Il est donc
