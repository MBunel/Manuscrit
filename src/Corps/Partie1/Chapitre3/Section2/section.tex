...
(\autoref{fig:zadeh_conorm}) et d'intersection
(\autoref{fig:zadeh_norm}) de \textcite{Zadeh1965}, ce qui permet de
...
La définition des opérateurs probabilistes est la suivante :
...
Le première particularité, visible sur la \autoref{fig:drast_op}, est
d'être \emph{discontinus,} contrairement aux opérateurs précédents.
...
particularité d'êtres \emph{duals} \autocite{Bouchon-Meunier2007}, ce
qui signifie que pour tous \(x\) et \(y\) dont la valeur est comprise
entre 0 et 1, \emph{la négation d'une \emph{t-norme}} est égale à la
\emph{t-conorme} de la négation de \(x\) et de \(y\) et inversement,
soit :
