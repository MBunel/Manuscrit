...
décomposition de l'ensemble des \emph{indices de localisation} et des
...
pas prévues à cet effet. C'est pourquoi nous proposons d'automatiser
l'étape de décomposition des \emph{relations de localisation
  atomiques.}
...
adossée à une méthode de \emph{spatialisation,} permettant de
construire la \emph{zone de localisation compatible} correspondant à
la \emph{relation de localisation atomique} pour un \emph{objet de
...
nécessaire de disposer de connaissances a priori sur la sémantique des
\emph{relations de localisation.}

Nous proposons donc de créer une base de règles, formalisant la
...
Dans l'exemple utilisé pour présenter le \emph{principe de
  décomposition} (cf. \ref{subsec:4-3-3-4}) une \emph{relation de
...
s'il est peu probable que ce nombre puisse atteindre de très grandes
...
Ainsi, pour représenter l'ensemble des décompositions possibles, il
est nécessaire que la base de règles définie puisse :
\begin{enumerate*}[label=(\arabic*)]
