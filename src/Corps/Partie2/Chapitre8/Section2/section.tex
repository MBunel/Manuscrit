...
et \(Π(B)\) soient égaux à 0). Une autre propriété des \emph{mesures
  de possibilités} est leur \emph{monotonie} relativement à
l'inclusion des parties de \(X\), c'est-à-dire que si un événement
\(A\) est inclus dans un événement \(B\), alors la possibilité de
\(B\) est supérieure à celle de \(A\) :
%
\begin{equation}
  A \subseteq B, Π(A) ≤ Π(B)
\end{equation}
%
De plus, tous les événements de \(X\) ou leur complémentaires sont
...
On peut remarquer, qu'à la condition qu'elle soit normalisée
\footnote{Contrairement à une \emph{distribution de possibilité,} une
  ...
  ces élément à un degré d’appartenance égal à un
  \autocite{Bouchon-Meunier2007}.}, une \emph{fonction d'appartenance}
(\autoref{sec:3-2}) est une \emph{distribution de possibilités.}
