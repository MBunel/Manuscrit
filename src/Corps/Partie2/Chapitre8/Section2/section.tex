...
\(A \subseteq B, Π(A) ≤ Π(B)\). Enfin, tous les événements de \(X\) ou
leur complémentaires sont \enquote{tout à fait possibles} :
\(∀ A ∈ P(X), \max(Π(A), Π(A^C)) = 1\), par conséquent, la somme des
possibilité des ces deux événements peut dépasser 1 :
\(Π(A) + Π(A^C) ≥ 1\) \autocite{Bouchon-Meunier2007}.
...
coefficient de possibilité on parle de mesure \enquote{totalement
  définie} \autocite{Bouchon-Meunier2007}. Pour attribuer une
\emph{mesure de possibilité} à chaque événement de \(X\) on défini une
\emph{distribution de possibilité,}
