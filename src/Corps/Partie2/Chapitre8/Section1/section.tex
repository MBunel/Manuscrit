...
\emph{t-conormes} partagent les mêmes propriétés. Parmi ces couples
deux sont archimédiens et deux ne le sont pas.
%
Tout d'abord les opérateurs drastiques ne sont pas archimédiens, leur
discontinuité ne le permettant pas.
%
Les opérateurs de \textcite{Zadeh1965} ne sont pas non plus
archimédiens, puisque l’application de la \emph{t-norme} (́\(⊤_Z\)) ou
de la \emph{t-conorme} de \bsc{Zadeh} (\(⊥_Z\)) à un couple du même
degré d'appartenance (\(x\)) est toujours égale a cette même valeur,
soit : \(∀ x ∈ [0,1]\): \(⊤_Z(x, x) = x\) et \(⊥_Z(x,x) = x\).


%
Comme on peut le voir sur ces deux figures, seuls les opérateurs de
\bsc{XXX} et les opérateurs probabilistes sont archimédiens.
...
éloignée des considérations principales de ce travail. Cependant,
cette propriété influe le résultat des intersections et des unions,
qui sont au cœur de notre méthode. Il est donc nécessaire d'en étudier
attentivement les conséquences. Comme nous l'avons expliqué ci-dessus,
