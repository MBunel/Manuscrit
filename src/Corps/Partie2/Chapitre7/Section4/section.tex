...
laquelle la courbe est centrée est fixé à 0. Nous avons également
défini une seconde variante de ce \emph{fuzzyfieur,} \onto{Eq\-Angle},
fortement similaire à \onto{Eq\-Val}, mais destinée aux cas
spécifiques où la \emph{métrique} est un angle. Ce \emph{fuzzyfieur}
permet donc de traiter ces valeurs en prenant en compte leur
périodicité. \todo{Ce sélecteur ne devrait pas exister, c'est à la
  fonction python d'identifier si ce qu'on lui donne est angulaire ou
  non, pas à l'ontologie. Voir si je l'enlève}.
...
valeur de référence est fixée à 0 a été définie, \onto{Sup\-Val\-0}.
...
Les trois types de \emph{fuzzyfieurs} définis ne permettent pas de
représenter toutes les configurations possibles.

Les \emph{modifieurs} sont des fonctions appliqués après la
\emph{fuzzyfication} et qui en modifient le résultat.
