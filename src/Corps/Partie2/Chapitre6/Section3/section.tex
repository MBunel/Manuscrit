...
des \emph{fonctions d'appartenance,} permettant d'attribuer à chaque position
un \emph{degré d'appartenance} à la \emph{zone de localisation compatible}
créée. Par conséquent la définition de ces fonctions est extrêmement
importante, puisqu'elle \enquote{fixe} la sémantique de la \emph{relation de
  localisation atomique} considérée.

Comme nous l'expliquions lors de la présentation de la \emph{théorie des
  sous-ensembles flous} (Section \ref{subsec:theorie_flou}) une fonction
d'appartenance ($f_A$)
