\begin{tikzpicture}
  ...
  \begin{scope}
    ...
    % 
    \begin{scope}
      \foreach \x [evaluate=\xshift using 1.5+\x/10, evaluate=\rad using (\x * -.0008) + .05] in {0,...,50}
      {
        \draw[fill=RdBu-9-9,draw=none, below] ([xshift=\xshift cm, yshift=-1.5cm]P1) circle [radius=\rad cm];
      }
      \foreach \x [evaluate=\xshift using 6.5+\x/10, evaluate=\rad using (\x * .0008) + .01] in {0,...,50}
      {
